...
3. [단계별 실습: 내 손으로 직접 만들고 검증하기](#3-단계별-실습-내-손으로-직접-만들고-검증하기)
   ...
   * [Step 4: 스크립트 실행 및 자유낙하 물리 정합성 검증](#step-4-스크립트-실행-및-자유낙하-물리-정합성-검증)
   * [Step 5: 인터랙티브 GUI 3D 뷰어 조작 실습](#step-5-인터랙티브-gui-3d-뷰어-조작-실습)
4. [스크립트 실행 결과 및 해석 가이드](#4-스크립트-실행-결과-및-해석-가이드)
5. [트러블슈팅 가이드 (자주 발생하는 오류 및 해결책)](#5-트러블슈팅-가이드-자주-발생하는-오류-및-해결책)
6. [Phase 01-U00 완료 체크리스트](#6-phase-01-u00-완료-체크리스트)
...
본 프로젝트에서는 시각화를 위해 두 가지 방식을 혼용합니다:

1. **대화형 뷰어 (`mujoco-python-viewer` 또는 `mujoco.viewer`):**
   * GLFW 기반의 데스크톱 3D 창을 띄웁니다.
   * 마우스 좌클릭(회전), 우클릭(이동), 휠 스크롤(줌)을 통해 로봇의 물리 동작을 사용자가 실시간 3차원으로 관찰할 수 있습니다.
2. **오프스크린 렌더러 (`mujoco.Renderer`):**
   * 화면에 창을 띄우지 않고, 그래픽 카드(또는 CPU OSMesa)의 메모리 버퍼(FBO)에 직접 RGB 영상과 Depth(거리) 영상을 렌더링합니다.
   * **D435i 비전 노드(Phase 04)와 단위 검증(U03)**에서 카메라 센서 데이터를 추출할 때 사용되는 핵심 엔진입니다.
...
### Step 4: 스크립트 실행 및 자유낙하 물리 정합성 검증
...
자동 검증이 모두 통과되면, `--viewer` 플래그를 붙여 직접 마우스로 샌드박스 월드를 둘러봅니다:
...
## 4. 스크립트 실행 결과 및 해석 가이드

정상적으로 실행되었을 때 터미널에 출력되는 표준 결과 예시입니다:
...
### 💡 물리 결과 해석:
* 자유낙하 거리 $h = 1.0\text{m} - 0.05\text{m} = 0.95\text{m}$
* 이론적 시간 $t_{\text{theory}} = \sqrt{\frac{2 \times 0.95}{9.81}} \approx 0.44007\text{s}$
* 시뮬레이션 측정 시간 $t_{\text{sim}} = 0.4400\text{s}$ ($dt=0.002\text{s}$ 기준 정확히 220 스텝 소요)
* 오차율 $0.02\%$로 고전 역학 자유낙하 운동과 수치적으로 완벽히 일치함을 확인하였습니다.

---

## 5. 트러블슈팅 가이드 (자주 발생하는 오류 및 해결책)
